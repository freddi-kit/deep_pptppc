...
class Stage1(chainer.Chain):
    def __init__(self, n_point):
        super(Stage1, self).__init__(
            ...
            conv7=L.Convolution2D(512, n_point, ksize=1, stride=1, pad=0),
        )
...
class StageN(chainer.Chain):
    ...
    def __init__(self, n_point):
        super(StageN, self).__init__(
            ...
            conv1=L.Convolution2D(32 + (n_point), 128, ksize=11, stride=1, pad=5),
            ...
            conv5=L.Convolution2D(128, n_point, ksize=1, stride=1, pad=0),
        )

    def __call__(self, pmap, fmap):
        fmap = self.conv0(fmap)
        fmap = F.relu(fmap)

        h = F.concat((fmap, pmap), 1)
        h = self.conv1(h)
        h = F.relu(h)
        h = self.conv2(h)
        h = F.relu(h)
